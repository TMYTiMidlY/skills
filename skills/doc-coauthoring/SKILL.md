...
- If images exist without alt-text, explain that images without alt-text may be inaccessible to some readers or tools. Ask if they want alt-text generated. If so, request they paste each image into chat for descriptive alt-text generation.
...
**If no integrations are detected:** Suggest they check if any connectors or integrations can be enabled to allow pulling context from messaging apps and document storage directly.
...
- If user mentions team channels or shared documents:
  ...
  - If integrations not available: Explain lack of access. Suggest they check if connectors can be enabled, or paste the relevant content directly.
...
Create file with all section headers and brief placeholder text like "[To be written]" or "[Content here]".
...
1. Open a fresh agent conversation (with no prior context)
