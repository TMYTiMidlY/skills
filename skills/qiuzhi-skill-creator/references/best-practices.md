...
本文档整合了 Agent Skills 的核心编写原则和最佳实践。
...
- **无障碍的开阔田野**：多条路径都能成功 → 给出大方向，信任 Agent 找到最佳路线（高自由度）
...
- 保留词: `anthropic-helper`, `agent-tools`
- 集合内命名不一致
...
保持引用**一层深度**。Agent 可能只部分读取嵌套引用的文件。
...
| 假设工具已安装 | "使用 pdf 库处理文件" | 明确依赖：`uv run --with pypdf` 而不是 `uv run`|
...
### Markdown 格式
- [ ] 符合 markdownlint 规范（VS Code markdownlint 扩展标准）
- [ ] 标题前后有空行
- [ ] 代码块用 ``` 包裹并标注语言
- [ ] 列表项格式统一
- [ ] 文件末尾有且仅有一个换行
...
1. **识别差距**: 在没有 Skill 的情况下让 Agent 执行代表性任务，记录具体失败
2. **设计测试提问**: 构建三个测试这些差距的提问
3. **建立基线**: 观察没有 Skill 时 Agent 的表现
