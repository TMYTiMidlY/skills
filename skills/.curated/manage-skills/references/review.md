...
先向用户确认审查范围（如原创、已适配嫁接、实验性、全部），然后对范围内每个 skill 逐条核对 [checklist.md](checklist.md) 里的每项要求，再加下面「跨 skill 专项检查」。默认**只审不改**：先列出发现交给用户，明确同意后才动手改。
...
[checklist.md](checklist.md) 里多是"单个 skill 自身要满足的属性"，看一个 skill 就能核。下面这几项不一样——只有把全仓摆在一起、或对照上游才做得出来，所以单独放在审查流程里，不塞进 checklist。
