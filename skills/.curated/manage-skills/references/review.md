...
- 根据文档引用块中给出的支撑信息和联网搜索，核验文档内容表述是否正确。
- 先打开引用块指向的原始来源，确认它确实支持相邻结论且版本、上下文一致；再联网搜索官方文档、规范、源码和发行说明等一手来源交叉核对。搜索结果摘要不能单独作为证据。
- 将结论记为"已证实 / 与证据冲突 / 证据不足 / 本仓约定"；发现冲突或事实修订时单独说明，不把内容更正伪装成普通润色。
