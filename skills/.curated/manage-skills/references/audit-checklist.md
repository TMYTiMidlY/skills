...
- **引用状态**
  ...
  - 若引用精确到文件内某个章节（`#anchor`），锚点应是嵌在标题里、与标题文字脱钩的显式 `<a id="...">`（英文 kebab-case），不能是标题文字自动派生的 slug（如 `#安装步骤`）——后者标题一改链接就悄悄失效。
...
- **标题命名**（详见 SKILL.md「标题命名」）
  - 标题命名主题 / 对象 / 类别，不预告结论 / 计数 / 实测值 / 优劣。逐条看标题有无这些病：计数入题（`…：33 个`）、结论入题（`…：服务端自报`）、排名入题（`—— 最简单`）、口语设问（`…发什么` / `…谁说了算`）、只有读完才懂的自造词 / 比喻（`地基` / `三个位置`）、给有具体主题的小节戴 `TL;DR` / `结论先行` 帽子。
  - 兄弟小节靠客观属性区分、不靠排名优劣；改带显式 `<a id>` 的标题时只改文字、不动锚点。
- **不硬编码仓库 clone 位置**
