...
- `graft-skill` 负责：从外部仓库引入 skill、上游同步、去品牌化适配、落点选择、`grafted-skills.json` 与来源栏维护。
- `graft-skill` 面向仓库维护 / skill 开发流程，不属于普通用户安装与使用场景。
...
- **外部来源迁入、上游同步、来源索引维护不属于本 skill 范围**，统一交给 `graft-skill`。
