...
## Copilot CLI 案例中的记录核查

以下方法用于核查整理范例时的 Copilot CLI 历史记录。其中的目录、SQL 表和事件字段只适用于 Copilot CLI，不能直接套到其他工具。实际盘点先按[会话来源与标识](../SKILL.md#session-source)确认来源；用户只给 ID 时仍须先询问工具名称。

1. **本地检索**——用 `asmgr search "<关键词>" --agent copilot --session <id>`（或跨会话 `asmgr search "<关键词>" --agent copilot`）检索本机存档。需要 SQL 时用只读客户端访问本机数据库，不调用云端会话检索。
...
5. **dump 当前 live session 验证 schema**——会发现 `turns` 表只有 user/assistant 文本，而完整时间线（工具调用 / reasoning / 通知）在 `~/.copilot/session-state/<id>/events.jsonl`。**直接看 events.jsonl，别只读 `turns` 表**——这是花了最久才意识到的一点，可以省掉。
   - **交互式提问 / 选择题的回答不是 `user.message`**，而是 `tool.execution_complete`（按 `toolCallId` 配对 start，答案在 `data.result.content` 的 `User selected/responded:`）。只数 `user.message` 必漏掉选择型决策。`asmgr` 现在把它抽成一等 `user/decision` 条目（`--format dialogue` 直接带上）；DB-only 会话（只有 `turns` 表）**无法**恢复这些决策。
