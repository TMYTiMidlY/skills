---
name: remote
description: 通过 SSH 远程操作已知 VPS/服务器/远程机器时使用；负责连接确认、远程命令执行、sudo 脚本上传、复杂文件修改等操作规范。涉及 sudo 时，agent 负责本地准备与 scp 上传，用户只执行最终的 ssh -t sudo 命令。
---
...
Windows 本机的 OpenSSH 通常不能用 ControlMaster，不需要满足这一项；只在 Linux / macOS / WSL 等支持连接复用的环境检查。
...
需要 sudo 的命令，将命令写入本地 `/tmp` 下的脚本，由 agent 自己 `scp` 到服务器，然后只让用户执行：

```bash
ssh -t <name> "sudo bash /tmp/<script>.sh"
```

`-t` 强制分配伪终端，使 sudo 能正常读取密码。

不要让用户执行 `scp`。上传脚本、上传临时文件、拉取日志等准备动作都应由 agent 完成；用户侧只保留最终那条需要交互输入 sudo 密码的 `ssh -t ...` 命令。
