...
3. **三档结论**：
   - ✅ **吻合**：与原文逐字一致 → 放过
   - ⚠️ **支撑偏弱**：文献存在但不直接支撑这个具体说法（如某文献 PDF 不直接支撑论文写的口径，只给了另一个范围/对象的数字）→ 标"待核"，说明原文能支撑到哪一层；仅加"据公开报道"或"据行业统计"不能补足证据
   - 🚨 **原文未载**：所引文献中找不到该数字或结论 → 标"待核"，记录查证范围和缺口，不能仅凭这一点断言该说法为假
4. **按证据与授权处理**：⚠️/🚨 项补找真正支撑的来源；仍无法支撑时，提出删去不受支持的细节或改为有证据支持的较弱结论等建议。仅核查时报告，不改成稿；已获准修订时在约定范围内执行，超出范围或实质改变结论再确认。✅ 项放过；核查结论与修改状态分别记录，不能因改了措辞就把待核项记为✅
5. 输出按 `SKILL.md`《核查输出格式》规范，**核验台账落项目根 `reference.md`**（逐数据点 `D01..` 的 ✅/⚠️/🚨 结论 + 一手出处），别只留在对话里

**强约束**：每条 ⚠️/🚨 必须列出已核查的来源位置及相关原文作为依据；未找到对应内容时，记录检索范围、关键词和缺口，不捏造不存在的关键句，也不凭印象判定。
...
4. **AI 配图未标注** —— 看似中性的"作者自绘"实际是 AI 生成；按 [AI 标注](figures.md#annotation) 加标
