...
- [Codex 订阅生图接入](references/image-gen.md)：先区分生成、编辑和请求提交方式，再说明遮罩制作、接口支持及模型参数；重点说明脚本直接调用 CPA、DSH 通过生图插件或 MCP 工具连接 CPA，含生成、编辑及遮罩请求示例、独立实例／Home 模式的并发行为，以及精简的结果处理和验收说明。
