...
- [DeepSeek Harness（dsh）运行时](references/dsh.md)：安装与 systemd 服务、Web 访问控制和会话凭据、Cordis 插件组合与本地 tarball 部署、Agent preset / loop / Session、模型 / Skills / MCP / Subagent、ACP / SDK。含 Copilot 账号端点与 Astra 协议修正、独立兼容路由的共享登录和工具回放、有界在线验证，以及 Host Plugin、工具沙箱、MCP 和遥测的信任边界。
...
- [Codex 订阅生图接入](references/image-gen.md)：先区分生成、编辑和请求提交方式，再说明遮罩制作、接口支持及模型参数；以 HTTP 代理、图片库、宿主插件和 MCP 介绍外部应用接入，含 CPA 调用示例、原图和遮罩提交、独立实例／Home 模式的并发行为，以及精简的结果处理和验收说明。
