...
- [DeepSeek Harness（dsh）运行时](references/dsh.md)：产品定位与安装、npm / pnpm 发布面与 systemd 用户服务，以及 Plugin / Context / Fiber、Bundle / Profile / Agent preset、配置合成和创造模式的运行时含义；另含 Agent loop、Session persistence、模型 / Skills / MCP / Subagent、ACP / SDK，以及 Host Plugin、工具沙箱、MCP 进程和遥测的信任边界。
- [DeepSeek Harness（dsh）Plugin 开发](references/dsh-plugin.md)：面向 Plugin 作者，区分 module、Loader entry、package 与开发入口，再按临时原型、源码工程、生命周期、Tool / Provider / 协议 / Web / Session 扩展、分层验证、打包安装和版本维护组织完整开发流程；运行时定义直接引用 dsh.md，不在本篇重复展开。
- [DeepSeek Harness（dsh）Plugin 调研记录](references/dsh-plugin-research.md)：维护现成 Plugin 与社区生态快照，覆盖桌面应用、TUI、市场主题、Tool / Provider 与业务扩展，并按日期保留专题检索的候选仓库、固定源码证据、阶段结论和未验证项；通用开发方法再整理进 Plugin 开发 reference。
