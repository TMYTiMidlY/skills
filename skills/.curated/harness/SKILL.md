...
- [Codex 运行时笔记](references/codex.md)：app-server 的进程内和独立运行形态、普通 CLI 在独立服务运行或未运行时的连接行为、服务启动、进程归属、程序升级和管理命令的执行环境；另含 `/usage` 云端账号活动图、额度窗口及第三方客户端的统计边界，上下文窗口、自动压缩、协作模式、内置生图工具和订阅登录凭据。
