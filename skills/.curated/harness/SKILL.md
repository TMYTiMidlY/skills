...
- [DeepSeek Harness（dsh）Plugin 调研记录](references/dsh-research.md)：维护现成 Plugin 与社区生态快照，覆盖桌面应用、终端界面、插件市场、浏览器文件传输、产物交付和模型 Tool 的请求与额度归属，以及底层服务、Web 界面和协作流程；按日期保留专题检索的候选仓库、固定源码证据、阶段结论和未验证项，通用开发方法再整理进 Plugin 开发 reference。
