...
- [Copilot CLI app.js 运行时补丁](references/copilot-patch.md)：**一键补丁脚本** `scripts/patch-copilot-cli.py`，按运行时结构选择新版 native 调用边界适配或旧版 JS 锚点。新版覆盖启动、主会话切换、选择器和非交互默认档位：effort 取**当前有权使用的最高档**，支持时选长上下文，默认模型标记跟随用户配置，保留显式选择；整组入口必须命中，写前校验、备份与原子替换。旧版另保留重试 `maxRetries` 5→10、四处 context 锚点与手动逆向记录；含 `N/A` / `SKIP` 边界、函数和 PTY 覆盖方法。（`web_fetch` fake-ip 补丁已退役。）
- [Copilot CLI 配置发现](references/copilot-discovery.md)：walk-up（向上查找）机制、Custom Instructions（指令文件）、Hooks、MCP 配置、Skills 发现。
- [Copilot 登录凭据与跨机 / 跨客户端复用](references/auth.md)：CLI / SDK / pi 的凭据存储、环境变量优先级和隔离；pi 的直接 key 与 OAuth 路径；账号 `endpoints.api`、token 类型和模型协议的分别判定。包含 421、`unsupported_api_for_model` 的诊断及低请求量复现；`gho_` 前缀或单次失败不能证明必须交换 token，账号地址也不能由 `internal` 或“企业”标签推断。
- [各客户端对 MCP 的支持](references/mcp.md)：讲 MCP 的协议模型（Anthropic 公告与官方 spec 口径、三类能力与控制方），再逐能力对照 Copilot CLI / Claude Code / Codex / Gemini CLI / Cursor（DSH 为第六对照点，细节指向 dsh.md）：Tools 的命名与前缀规则（六家都取本地配置的 server 标识）、Resources / Prompts 的消费入口（@ 引用 vs 工具桥接 vs 无）、工具结果内容块的各客户端处理、配置发现与枚举修改；区分闭源实测、官方文档与开源源码证据。
...
- [DeepSeek Harness（dsh）运行时](references/dsh.md)：产品定位与安装、npm / pnpm 发布面与 systemd 用户服务、Web 访问控制与会话凭据、Cordis 插件框架概览与现成 Plugin 的安装卸载、Agent preset（含创造模式）、Agent loop、Session persistence、会话统计条（轮／步、计时、输入输出、缓存命中与子代理／fork 口径）、模型 / Skills / MCP / Subagent、ACP / SDK，以及 Host Plugin、工具沙箱、MCP 进程和遥测的信任边界。
- [DeepSeek Harness（dsh）Plugin 开发](references/dsh-dev.md)：Cordis 插件框架的具体结构与开发主入口——Plugin / Context / Fiber、module / Loader entry / package、Bundle / Profile / Agent preset、配置合成与加载路径、Loader entry 的 id / insert / 按 id 覆盖，以及临时原型、源码工程、模型可调用操作、底层服务、协议、Web 界面和 Session 扩展、分层验证、打包安装和版本维护。
- [DeepSeek Harness（dsh）Plugin 调研记录](references/dsh-research.md)：维护现成 Plugin 与社区生态快照，覆盖桌面应用、终端界面、插件市场、浏览器文件传输、产物交付、模型 Tool 的请求与额度归属，以及 `web_search` 搜索提供方（官方 web 包家族与社区 provider 生态）、底层服务、Web 界面和协作流程；按日期保留专题检索的候选仓库、固定源码证据、阶段结论和证据边界，通用开发方法再整理进 Plugin 开发 reference。
- [DSH Model Hub：订阅登录、版本边界与额度查询](references/dsh-model-hub.md)：核对官方 `llm-pi-ai` 与内建 `codex` / `qwen-code` 的归属、当前 Models 插槽和依赖/目录差距；比较 Codex App Server、Host usage reader 与现成订阅插件，明确单一凭据刷新 owner、只读额度契约及验收边界。
...
- [Codex 运行时笔记](references/codex.md)：app-server 的进程内和独立运行形态、普通 CLI 在独立服务运行或未运行时的连接行为、服务启动、进程归属、程序升级和管理命令的执行环境；另含[桌面端 projectless 目录与提示词](references/codex.md#desktop-projectless)（原始函数、完整 prompt 和开源证据边界）、`/usage` 云端账号活动图、额度窗口及第三方客户端的统计边界，上下文窗口、自动压缩、协作模式、内置生图工具和订阅登录凭据。
- [Codex 订阅生图接入](references/image-gen.md)：先区分生成、编辑和请求提交方式，再说明遮罩制作、接口支持及模型参数，独立对照代表项目的接入方式和验证范围；重点说明脚本直接调用 CPA、DSH 通过生图插件或 MCP 工具连接 CPA，含生成、编辑及遮罩请求示例、独立实例／Home 模式的并发行为，以及精简的结果处理和验收说明。
