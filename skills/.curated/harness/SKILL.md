...
- [Codex 订阅生图接入](references/image-gen.md)：Images、Responses 和 Agent 包装的调用路径，编辑输入、公共 API 参数对照、请求生命周期、产物交付和组件比较；含 CLIProxyAPI 的本机配置、生成、JSON／multipart 编辑、官方客户端调用及验收，区分固定源码、历史实测和未知能力。
