...
- [DeepSeek Harness（dsh）运行时](references/dsh.md)：产品定位与安装、npm / pnpm 发布面与 systemd 用户服务、Web 访问控制与会话凭据、Cordis 插件框架概览与现成 Plugin 的安装卸载、Agent preset（含创造模式）、Agent loop、Session persistence、模型 / Skills / MCP / Subagent、ACP / SDK，以及 Host Plugin、工具沙箱、MCP 进程和遥测的信任边界。
...
- [Codex 运行时笔记](references/codex.md)：TUI / exec 的 app-server 连接路径、版本与 PID 管理归属、稳定版与主线的关停和自动更新差异；旧后台升级事故中的 systemd 内联脚本展开、超时误归因和验收证据。另含上下文窗口、自动压缩、协作模式和结构化提问；内置生图工具的启用条件、请求构造、effort 作用范围、耗时统计、公共 API fallback，以及订阅登录凭据的存储、刷新和跨程序复用。
