...
- [Agent Skills 规范与各 harness 的 skills 加载对照](references/skills-loading.md)：官方规范仅 6 个 frontmatter 字段、无层级机制；官方生态的分组全是平铺式（平铺目录 / 插件命名空间 / monorepo 嵌套发现）；`has-sub-skill` 是 Kimi Code 私有扩展而非社区规范，跨工具移植时保持平铺最兼容。
