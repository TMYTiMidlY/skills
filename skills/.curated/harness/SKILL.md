...
- [DeepSeek Harness（dsh）运行时](references/dsh.md)：产品定位与安装、npm / pnpm 发布面与 systemd 用户服务、插件化思路与现成 Plugin 的安装卸载、Agent preset（含创造模式）、Agent loop、Session persistence、模型 / Skills / MCP / Subagent、ACP / SDK，以及 Host Plugin、工具沙箱、MCP 进程和遥测的信任边界。
- [DeepSeek Harness（dsh）Plugin 开发](references/dsh-plugin.md)：插件系统具体结构与开发主入口——Plugin / Context / Fiber、module / Loader entry / package、Bundle / Profile / Agent preset、配置合成与加载路径，以及临时原型、源码工程、Tool / Provider / 协议 / Web / Session 扩展、分层验证、打包安装和版本维护。
