# Copilot CLI discovery, instructions, hooks, and skills

How Copilot CLI discovers project/user context: walk-up behavior, custom instructions, hook configuration, and skill directories. Use this when debugging why a project-level instruction, hook, or skill is or is not loaded.
...
`yQe` 在 `convention="directory"` 模式下转调 `bBt`（与 [Copilot MCP 里的 `.mcp.json` walk-up](copilot-mcp.md#mcpjson-上溯停在-git-root多-repo-工作区漏装) 是同一类机制）：
