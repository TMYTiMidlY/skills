...
- **effort 叫 "thinking level"**（`off|minimal|low|medium|high|xhigh|max`），映射到各家不同的线格式（Codex `reasoning.effort`、Anthropic `output_config.effort` 或 `thinking.budget_tokens`…），不是统一的 `reasoning_effort`。[^effort]
...
七档 `off | minimal | low | medium | high | xhigh | max`，经每模型的 `thinkingLevelMap` 映射到各家线格式——**不是统一的 `reasoning_effort`**：Codex Responses → `reasoning.effort`；Anthropic 自适应模型 → `output_config.effort`；老 Anthropic → `thinking.budget_tokens`；OpenAI 兼容各家 → 若干变体（部分才是 `reasoning_effort`）。`xhigh`/`max` 仅在模型显式映射时暴露，不支持的档会被 clamp。[^effort]
