# coding agent 的安装与分发形态（Codex / Claude Code / Copilot CLI）
...
## <a id="packaging-survey"></a>打包形态普查（`npm view` 实测）
...
### <a id="libc-split"></a>libc 分叉（glibc / musl）
...
### <a id="toolchain-evidence"></a>真身工具链的判据

下面三条都不是从文档推断，而是**下载各家平台二进制、直接查看文件内容**实测确认的（由 Sonnet 5 实操）。
...
## <a id="install-entries"></a>安装入口（脚本直装 / Homebrew / winget / npm）

真身既是不依赖 Node 的原生程序，厂商现在都**主推"脚本直装 / Homebrew / winget"直接拉二进制**，`npm` 退居二线、多作兜底 / 兼容入口。下表照各家官方 README 核对：
...
- 这些工具本身就是"自带升级、不依赖任何运行时的独立原生程序"，用官方脚本 / brew 一步装好最省事（还免去先装 Node）——这正是"能进包管理器就优先包管理器"这条通则的**边界情形**（见 `software` skill 的"绕过包管理器的安装"章节）。

## <a id="soft-deprecation"></a>npm 的软弃用（soft deprecation）
...
## <a id="adoption-snapshot"></a>采用度快照（agent 侧，2026-06-29 … 07-05）
...
- pnpm / yarn / bun 等**包管理器客户端**自身的采用度与社区风向，属包管理器话题，见 `software` skill 语言级章节里的客户端采用度快照。
