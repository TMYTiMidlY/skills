...
测量目标是区分无线一跳、校园/企业内部网络、公共互联网和测速服务器。一次只改变一个变量，并保留相同设备、认证、目标和样本大小。
...
使用内容下载可以把“无线/接入层慢”和“公共出口慢”分开。选择内部镜像与外部镜像上大小相同或同源的静态文件，固定样本长度，并在同一时段连续测试：
...
外部目标使用相同 Range 和文件大小。若内部下载已经很慢，瓶颈位于无线或接入层；内部快、外部慢时，再检查公共出口和外部路径。
...
eduroam 运行期间，日志出现 AP发出的 WNM `Disassociation Imminent`，并发生同一 AP能力从 VHT 变为 HT/legacy 后由本机 mac80211 驱动断开、重新选 AP、EAP 和 DHCP 的事件。两次完整恢复约需 5–6 秒；没有真正断线的 WNM 事件也伴随 150–220 ms 延迟尖峰和高重传。
