...
不同系统读取链路状态的接口不同。下面以 OpenWrt 为例，先读取不改变关联的状态：
...
不同系统提供的日志、API 和保存周期不同。OpenWrt 的具体命令、历史边界和远程保存方法见 [链路日志与监控面板](openwrt.md#link-dashboard)。
...
## <a id="dashboard"></a>链路监控需求

监控的目标是同时看见无线状态、当前业务占用和主动基准测试，并明确区分实时值、历史值和不可用状态：

| 展示项 | 数据来源 | 语义 |
|---|---|---|
| signal/noise/SNR | 无线驱动或设备状态接口 | 当前接收与噪声 |
| TX/RX PHY | 当前关联状态 | 当前协商档位 |
| 当前流量 | 相邻接口字节计数差值 | 接口当前占用，不是可用带宽 |
| 实时 RTT（往返时延）/loss（丢包率） | 小样本 ping | 当前公网路径状态 |
| NDT7 | 主动基准测试 | 按需测下载、上传、负载延迟和重传 |
| 最近扫描 | 无线扫描缓存 | 候选网络可见性，不代表可用性 |

0–100 信号分数只能作为可配置的展示映射，不能当成行业标准。状态接口超时或字段缺失时，应显示 OFFLINE、STALE 或不可用，不用 `0` 伪装测量结果；主动扫描、延迟精测和 NDT7 还需要调度，避免监控本身干扰正在测量的链路。

把面板直接运行在路由器上，需要系统允许安装 Web 服务、采集程序和后台任务。当前模板及其 `uhttpd`、`ubus`、`iw/iwinfo` 数据接口属于 OpenWrt 实现，见 [链路日志与监控面板](openwrt.md#link-dashboard)。
