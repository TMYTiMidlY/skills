...
## 4. 运行态控制与连接排障：REST API、链路测试与 Web 面板
...
### 4.1 节点连接失败的分层诊断（CONNECT 200 / TLS EOF / 长连接）
...
3. **拿落地真实 IP，绕开本机代理测入口**：DNS 必须走 DoH（见 §7.5），避免 fake-ip；随后清除 proxy env、加 `--noproxy '*'`，并用 `--resolve` 同时钉真实 IP、保留正确 Host/SNI：
