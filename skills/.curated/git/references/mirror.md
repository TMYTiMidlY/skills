...
> ⚠️ **不作首选,保留存档。** 自建 forge(见 [forge.md](forge.md))能力更完整、维护面更小,还顺带带来 issue / PR / CI;纯粹"拉不下来"的场景多数可以用[受限网络获取](clone.md)那套代理与离线传输绕过,不必常驻一台中转机。本方案的成本在于:双向同步要自己写 hooks 防回环,两端并发推送时还有 split-brain 风险。已经跑着的可以继续用,新搭建先考虑那两条路。
