...
本文与本 skill [surgery.md 的 jj 小节](surgery.md#jj-no-index) 同源但侧重不同：那边把 jj 当作"git 精准手术做不到时的另一条路"来对照；这里是 jj 模型的完整介绍。**下文标"实测"的行为均在 jj 0.43.0 上验证**；官方文档 / 源码链接锁到 v0.43.0。
