...
> Copilot CLI 相关的调研笔记已迁移到 `software` skill 的 [references/copilot.md](../software/references/copilot.md)（包括 bash 工具 env 黑名单、`COPILOT_ALLOW_ALL` vs `--yolo`、`/rewind` 非 git 拒绝、Safety Net 双 bug、项目级 hook 不向上查、`.mcp.json` 上溯停在 git root、`.mcp.json` headers `${VAR}` 不展开、`GIT_CONFIG_COUNT` 注入 credential helper、`gh repo fork` SSH 身份错配等）。
