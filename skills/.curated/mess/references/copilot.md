...
1. **Parent 是 git repo、子项目也各自是独立 git repo**：`~/TiMidlY-projects/.mcp.json` 注册了 `portal`，在 `~/TiMidlY-projects/` 直接 `copilot mcp list` 能看到，但 `cd ssh-remote-mcp && copilot mcp list` 显示 `No MCP servers configured`。
