...
> 2026-05-09 首记 / 2026-05-11 补做边界测试 | Copilot CLI 1.0.44 | Linux

### 症状

两个反向场景都会出现"父目录 `.mcp.json` 看不到"：

1. **Parent 是 git repo、子项目也各自是独立 git repo**：`~/TiMidlY-projects/.mcp.json` 注册了 `portal`，在 `~/TiMidlY-projects/` 直接 `copilot mcp list` 能看到，但 `cd portal-mcp-server && copilot mcp list` 显示 `No MCP servers configured`。
2. **Parent 不是 git repo、子项目是 git repo**：`~/TiMidlY-projects/.mcp.json` 含 `portal`+`github-mcp-server`，子目录 `qcfd2026-site/.mcp.json` 含 `playwright`。在 `qcfd2026-site/` 起 copilot 只看到 `playwright`，parent 那两个继承不到。
...
`app.js`（`~/.local/share/fnm/.../node_modules/@github/copilot/app.js`）里的发现函数 `bBt`、合并函数 `bLa`、顶层调度 `sN`：
...
direnv symlink 套路（跟 `safety-net.json` 同款）：
...
- 实测才信结论；第一次只看 `copilot mcp list` 就断言"完全不上溯"是错的——读了 `app.js` 才发现它**会**上溯，只是停在 git root；之后又一度以为"没 git root 就会自由上溯"，再读一次 `sN` 才发现没 git root 反而连 parent 都不看，只读 cwd。
- `copilot mcp list` 输出的 `Source:` 列：`Project (.mcp.json)` 是 cwd 那份，`Inherited (...)` 是上溯命中的祖先那份，可以用来快速验证 symlink/合并实际生效到了哪。
- 同名 server 冲突时是 last-wins，cwd 覆盖 inherited；想强制不被父目录覆盖，server 名字起独特一点。
...
`~/.copilot/config.json` 的 `trustedFolders` 已经包含 `/home/<user>`（覆盖整棵 home 树），按直觉子目录任意上溯访问都不应该被拦。但从 `~/TiMidlY-projects/<subdir>` 启动 copilot 后，agent 读 `~/TiMidlY-projects/.safety-net.json`（在启动 cwd 的**父目录**）时仍弹：

```
Allow directory access
This action may read the following path outside your allowed directory list.
  /home/<user>/TiMidlY-projects/.safety-net.json
1. Yes
...
1. **先核对 `trustedFolders`**：`/home/<user>` 确实在里面（`config.json` 和 `settings.json` 两份镜像都有），按官方文档 "Trusted directories control where Copilot CLI can read, modify, and execute files" 的字面意思应该覆盖整棵子树。但实际并没有。
...
- 在 `/home/<user>/TiMidlY-projects/<subdir>` 启动后，会话 allowed-dir = `{<subdir> 及子树}`，不含 `<subdir>` 的兄弟和父目录。
- 哪怕 `trustedFolders` 写了 `/home/<user>`，只能让启动时不弹"trust this folder"信任 prompt，**不会**扩展会话 allowed-dir。
...
2. **启动后立刻 `/add-dir`**：
   ```
   /add-dir /home/<user>/TiMidlY-projects
   ```
   只对当前 session 有效。
3. **弹窗里选 2**：等价于上一条。
...
在 `~/TiMidlY-projects/` 放了 `.safety-net.json`，自定义规则禁止所有 `gh` 子命令（要求用 GitHub MCP server 代替）。但子目录 `portal-mcp-server/` 是独立 git repo，Copilot 的 cwd 在子仓库里时 safety-net 只从 **cwd** 读 `.safety-net.json`，不向上遍历——规则不生效。
...
```bash
cd ~/TiMidlY-projects/portal-mcp-server
...
#### 三、`--allow-all` 与 `--yolo` 完全等价，且都是"三合一"展开

CLI 选项注册段（offset 14164070 / 14164186）：
...
帮助文本一字不差。运行时 `txr()`（offset 12477961）把它俩 OR 到同一个变量：
...
- 如果你的 launch cwd 已经在 `trustedFolders` 子树下（比如 `/home/<user>` 在里面），那 isFolderTrusted 本来就过——`=1` 和 `=true` 没区别，你看不到副作用。
...
不能在 `.envrc` 里写 alias——direnv 只把 `.envrc` 中 `export` 出来的 env var 注入回父 shell，bash 函数和 alias 都会丢（它们是 shell 内部状态，不在 environ 里）。三种实操方案：
...
适合"只在某个工作区开 yolo"的场景。在工作区里建一个 wrapper 脚本：
...
进入这个目录时 `which copilot` 指向 wrapper；离开时 direnv 把 `.bin` 从 PATH 撤回，`copilot` 恢复原状。

**方案 C：env var + alias 组合（不推荐，但能理解为啥老配置长这样）**
...
权衡：`--allow-all-paths` / `--yolo` 让 agent 能读到 `~/.ssh/`、`~/.config/` 之类，确实有风险。不想全开就接受目录弹窗、必要时 `/add-dir` 临时加白。
...
- **`--allow-all` 和 `--yolo` 完全等价**——源码 `txr()` 里 `t?.allowAll || t?.yolo` 直接 OR 到同一个变量，再 OR 进 tools/paths/urls 三个 bool。CLI 没有任何"yolo 比 allow-all 更狂"之类的差别，纯粹只是别名。
- **alias 不能写进 `.envrc`**——direnv 只把 `export` 出来的 env vars 注入回父 shell，shell 函数和 alias 都会丢。想做"目录粒度的命令包装"得用 PATH-shim：在 `.envrc` 里 `PATH_add "$PWD/.bin"` + 项目内放可执行 wrapper 脚本（注意脚本里要把自身目录从 PATH 摘掉再 `exec`，避免无限递归）。
