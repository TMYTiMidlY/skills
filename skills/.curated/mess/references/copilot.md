...
1. **Parent 是 git repo、子项目也各自是独立 git repo**：`~/TiMidlY-projects/.mcp.json` 注册了 `ssh-remote`，在 `~/TiMidlY-projects/` 直接 `copilot mcp list` 能看到，但 `cd ssh-remote-mcp && copilot mcp list` 显示 `No MCP servers configured`。
2. **Parent 不是 git repo、子项目是 git repo**：`~/TiMidlY-projects/.mcp.json` 含 `ssh-remote`+`github-mcp-server`，子目录 `qcfd2026-site/.mcp.json` 含 `playwright`。在 `qcfd2026-site/` 起 copilot 只看到 `playwright`，parent 那两个继承不到。
