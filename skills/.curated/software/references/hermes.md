...
### config.yaml 与 .env 优先级（terminal.* 系列）

`terminal_tool.py` 通过 `os.getenv("TERMINAL_*")` 读取运行时值。`.env` 被加载了三次，最后一次在 yaml→env 同步之后，因此 **`.env` 优先于 `config.yaml`**：

1. `main.py` L154：`load_hermes_dotenv(override=True)` → `.env` 加载
2. `cli.py` L80：`load_hermes_dotenv(override=True)` → `.env` 加载
3. `cli.py` L574：`load_cli_config()` → yaml→env 同步（`config.yaml` 的值写入 `os.environ`）
4. `run_agent.py` L54：`load_hermes_dotenv(override=True)` → `.env` 第三次加载，**覆盖步骤 3**

受影响字段（`cli.py` 的 `env_mappings`）：`TERMINAL_ENV`、`TERMINAL_TIMEOUT`、`TERMINAL_LIFETIME_SECONDS`、`TERMINAL_DOCKER_IMAGE`、`TERMINAL_DOCKER_VOLUMES`、`TERMINAL_CONTAINER_CPU/MEMORY/DISK`、`TERMINAL_CONTAINER_PERSISTENT`、`TERMINAL_DOCKER_MOUNT_CWD_TO_WORKSPACE`、`TERMINAL_PERSISTENT_SHELL`、`TERMINAL_SSH_*`、`TERMINAL_SANDBOX_DIR`、`SUDO_PASSWORD`。

不受影响（直接从 config dict 读取，不走 env var）：`model.*`、`agent.*`、`compression.*`、`terminal.docker_env`、`terminal.env_passthrough`、`display.*`、`skills.*` 等。

实操：排查 `.env` 里的 `TERMINAL_*` 字段，**凡是 `config.yaml` 已有对应的就注释掉**，让每个值只在一处设置。`.env` 只保留 `config.yaml` 里没有对应字段的（如 `TERMINAL_SSH_*`）。`hermes config set` 的 `_config_to_env_sync` 映射表不完整（如缺 `lifetime_seconds`），不能依赖它同步。

### Docker backend

字段默认值即官方推荐（源码 `hermes_cli/config.py:386-408`）：镜像 `nikolaik/python-nodejs:python3.11-nodejs20`，资源 `container_cpu/memory/disk = 1 / 5120MB / 51200MB`，`container_persistent: true`，`persistent_shell: true`，`docker_mount_cwd_to_workspace: false`（开启会把宿主机 cwd 挂进容器、削弱隔离）。
...
#### lifetime_seconds 与 container_persistent 配合

`lifetime_seconds`（默认 300s）是容器空闲超时：后台 cleanup 线程每 60 秒扫一次 `_last_activity`，超过阈值就触发清理。`_last_activity` 只在 terminal tool 被调用时刷新——agent 思考、读文件、搜网页等操作**不续活**。

cleanup 时 `container_persistent` 决定删到什么程度：

| | persistent=false | persistent=true |
|---|---|---|
| 容器进程 | stop + rm | stop（不 rm） |
| sandbox 目录（/root、/workspace 的 bind mount 源） | 删除 | **保留** |
| 容器 overlay 层（apt/pip/npm 安装的包） | 丢 | **也丢** |

persistent=true 下，cleanup 后容器进程终止但 sandbox 目录保留在宿主磁盘。下次起新容器会 mount 同一 sandbox 目录，`/root` 和 `/workspace` 下的文件还在。但容器 overlay 层的安装（apt/pip/npm）随容器 stop 丢失。

cleanup 对用户通常**透明**：下次 terminal 命令来了 hermes 静默起新容器，用户只会发现 `hostname` 变了、之前 apt install 的包不在了。
...
**反面教材（踩过）**：`TERMINAL_ENV=ssh hermes chat ...` 这种临时 shell 环境变量 override **不生效**。原因有两层：（1）`cli.py` L498-506 的 yaml→env 同步会覆盖它；（2）即使侥幸过了第一层，`run_agent.py` L54 的 `load_hermes_dotenv(override=True)` 也会用 `.env` 里的值再次覆盖。backend 切换要改 `.env`（实际优先），或用 `hermes config set`（自动同步 yaml 和 `.env` 两边）。
...
`hermes skills list` 每条的 `Source` 字段标明来源，生命周期不同，容易踩混：

| 来源 | 怎么进来的 | 装卸方式 |
|---|---|---|
| `builtin` | 随 Hermes 发行版打包 | 命令管不了；`hermes skills reset <name>` 只清"user-modified"标记 |
| `hub` | `hermes skills install` 拉下来的 | 走 `install` / `uninstall`，状态记在 `~/.hermes/skills/.hub/lock.json` |
| `local` | 用户自己摆进 `~/.hermes/skills/<cat>/<name>/` 或通过 `external_dirs` 引入的 | 靠文件系统管理，`hub` 之外全归这一类 |

### install / uninstall 的要点

- `hermes skills install <identifier>`，identifier 是 `<registry>/<path>/<name>` 多段命名：
  - `skills-sh/<owner>/<repo>/<name>`：**skills.sh 是第三方索引站**（从 GitHub 仓库批量收录），不是 Hermes 官方。上游是可信仓（如 `anthropics/skills`）会标 `trusted`，其余 `community`。
  - `official/<category>/<name>`：Hermes / NousResearch 自家的官方 hub。
  - 只写 skill 名：会在所有 registry 里模糊搜。
- 装前会 quarantine + 安全扫描，给 verdict（`CAUTION` 常见，`BLOCKED` 要 `--force`）。
...
`~/.hermes/config.yaml` 的 `skills.external_dirs: []` 是 list，支持 `~` 和 `${VAR}` 展开，不存在的路径静默跳过（`agent/skill_utils.py:get_external_skills_dirs`）。

规则：

- 目录结构是 `<external_dir>/<skill-name>/SKILL.md`，skill 目录直接挂在 external_dir 下一层。
- **只读**：agent 创建 / 编辑 skill 只会写到 `~/.hermes/skills/`，不会污染源仓库。
- 同名遮蔽：`~/.hermes/skills/` 里的本地版 > external 版。
- 典型用法：把 `~/.agents/skills` 这类跨工具共享目录加进来，一次配好。

### 按项目启用 skill：没原生方案

Hermes **不做 walk-up 发现**——不会自动读项目根的 `.agents/skills/` 或 `.hermes/skills/`。能按 cwd 生效的只有 `.hermes.md / AGENTS.md / CLAUDE.md / .cursorrules` 这类**上下文文件**，那只是塞进 system prompt 的文本指令，和 skill 加载无关。

最接近的近似做法：

- `external_dirs` 写 `${PROJECT_SKILLS}` 占位，项目根 `.envrc`（direnv）或 wrapper 脚本里 `export PROJECT_SKILLS="$PWD/.agents/skills"`。启动那一刻的 shell 环境决定了扫哪些；进程跑起来后再 `cd` 不生效。
- 要完全隔离就走 profile：`HERMES_HOME="$PWD/.hermes-profile" hermes chat`，配合那个 profile 自己的 `config.yaml.external_dirs`。

### Backend 差异（跨 backend 迁移必看）

skill 文件怎么进容器 / 远端取决于 backend，关键差别在**对 symlink 的处理**：

- **Docker**：`get_skills_directory_mount()` 把本地 skills dir 和每条 external_dir 都 bind-mount 进去（`-v <host>:<container>:ro`，external 挂到 `/root/.hermes/external_skills/<idx>`）。**遇到 symlink 会创建临时净化副本**，安全但容器里看的不是原位置，对源文件的实时改动不会穿透（得重启 session）。
- **SSH / Modal / Daytona**：走 `iter_skills_files()` 逐文件 rsync / 上传，**直接跳过 symlink**。靠 symlink 接上的 skill 在这些 backend 上**完全不可见**。

结论：想跨 backend 通吃，就用 `external_dirs` 指向真实目录，不要在 `~/.hermes/skills/` 里架 symlink。

### `--skills` / `-s`：预加载

默认 Hermes 对 skill 走**渐进式披露**——system prompt 里只塞一份索引（name + description），agent 要用了才调 `skill_view` 把 SKILL.md 正文拉进来。`--skills <name>` / `-s <name>` 跳过这一层：

- 启动时直接把 SKILL.md 全文拼进 system prompt
- 另附 `[SYSTEM: user launched this CLI with "<name>" preloaded. Treat its instructions as active guidance...]` 强制激活句（`agent/skill_commands.py:build_preloaded_skills_prompt`）
- `references/` 子文档**不**随 preload 进来，仍按需拉

用法：`hermes chat -s github-pr-workflow`，多个用逗号或重复 flag。用在"本次明确要按某个 skill 办"的场景；让模型自己判断就别 preload，省 token。
