...
- **pixi**：conda-forge 生态的**现代前端**（Rust 写、快），`pixi.toml` + `pixi.lock`，项目级 + `pixi global`。见 [pixi.sh](https://pixi.sh/)；`pixi global` 的清单模型、trampoline、升级与临时环境等操作细节见 [pixi.md](pixi.md)。
