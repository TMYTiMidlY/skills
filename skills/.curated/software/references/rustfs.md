...
> RustFS 是 Rust 写的 S3 兼容对象存储（Apache 2.0，[github.com/rustfs/rustfs](https://github.com/rustfs/rustfs)），协议层照搬 MinIO 行为。本文是对接 RustFS 时踩过坑后的稳态结论：客户端怎么选、versioning 桶的软删/硬删/恢复语义、一个已修复的列举静默截断 bug、跨桶 copy 的 timeout 调参、以及 HDD 后端"对象数 ≫ 字节数"的性能特征。
>
...
- **mc 和 boto3 在不同 API 路径上各有坑**（尤其下面 §4 的列举 bug、§5 的 copy timeout），建议**混用 + crosscheck**，速查见 §7。
...
设了规则后，RustFS 后台 scanner 周期跑，遇到符合条件的 noncurrent 就物理删；**没设规则 = scanner 不删任何东西 = 旧版本永久攒着**（也是下面 §4 那个 bug 的温床——delete marker 越攒越多）。

## 4. ListObjectsV2 在 versioning 桶上静默截断（issue [#3217](https://github.com/rustfs/rustfs/issues/3217)，已修复）
...
versioning 桶里某 prefix 下，`ListObjectsV2` 返回 HTTP **200** + `IsTruncated=false` + `KeyCount` 远小于真实值（**有时直接 0**）+ **无 `NextContinuationToken`**。HTTP 200 + 谎称"列完了" = 静默失败，客户端拿不到任何"列不全"的信号。

```
<ListBucketResult ...><Name>...</Name><Prefix>shard1/</Prefix>
  <MaxKeys>1000</MaxKeys><KeyCount>0</KeyCount><IsTruncated>false</IsTruncated>...
</ListBucketResult>
```

### 触发条件

versioning 桶 + 大量 delete marker，且**顶版本是 delete marker 的"垃圾键"在字典序上排在可见对象前面**。control experiment：把可见键改名排到垃圾键前面，同样数据 bug 就消失——所以触发跟**字典序**强相关。根因一句话：服务端遍历磁盘时有个"读多少条就够了"的预算，delete-marker-only 的条目本不该算进这个预算却被算了，于是预算在还没走到可见对象时就耗尽，提前判定"列完"。

常见诱因：桶迁移、ETL 重写、失败重试的写路径，以及任何**没配 NoncurrentVersionExpiration 清 marker** 的 versioning 桶。

### 重要纠正：没有任何客户端能自动绕过（包括 mc）

> 早期笔记写过"mc 不信 IsTruncated、用 StartAfter 接力所以能看全"——**这是错的**。issue #3217 的 `mc ls --debug` 实测：mc 对 `ListObjectsV2` 只发一次请求、不补 `start-after` / `continuation-token`，照样少列。

| 客户端 / 命令 | 在该 bug 下 |
|---|---|
| boto3 / aws-cli / 各语言 SDK paginator | 严格信 `IsTruncated=false`，看到就停 → 少列 |
| `mc ls` / `mc ls --recursive` | 也只发一次 ListObjectsV2、不接力 → 少列 |
| `mc mirror` | full-miss prefix 上直接 `Unable to stat source` 放弃；partial-miss 上静默漏传 |

那"为什么以前以为 mc 能看全"？因为当时跑的是 `mc ls --versions`——那走的是**另一条 `ListObjectVersions` 端点**，跟 V2 列举不是一套代码，分页正确。

### 旧版本上唯一可靠的枚举 = 走 ListObjectVersions

```bash
# mc：列所有版本（含 marker），再自己过滤出"顶版本不是 delete marker"的键 = 可见集
mc ls --versions <alias>/<bucket>/<prefix>/
```

```python
# boto3：不同 endpoint，分页正确
paginator = s3.get_paginator('list_object_versions')
for page in paginator.paginate(Bucket='X', Prefix='Y/'):
    for v in page.get('Versions', []): ...        # PUT 版本
    for m in page.get('DeleteMarkers', []): ...   # delete marker
```

- 已知 key 时，`head_object(Bucket,Key)` / `mc stat` 单对象探测永远准（不走列举分页）。
- **手动 `StartAfter` 接力只能救 partial-miss、救不了 full-miss**：partial（`KeyCount=N>0`）时拿最后一个 key 当 `StartAfter` 再发一次能捞回剩下的；但 full-miss（`KeyCount=0`）连"最后一个 key"都没有 → 只能靠 ListObjectVersions。

### 诊断"是不是中招了"

手发一次原始调用，对照 `mc ls --versions` 的真实数：

```python
resp = s3.list_objects_v2(Bucket='X', Prefix='Y/', MaxKeys=1000)
print(len(resp.get('Contents', [])), resp.get('IsTruncated'),
      resp.get('NextContinuationToken', '(none)'))
# IsTruncated=False 但条数明显少于 mc ls --versions 过滤出的可见数 → 中招
```
...
|---|---|---|---|
| `ListObjectsV2`（列对象） | 受 §4 bug 影响，**不会自动接力** | 同样受影响，严格信 IsTruncated | 旧版本上**两者都不可靠**，改走 ListObjectVersions；新版本任意 |
...
| `mc mirror` 批量复制 | versioning 桶大 prefix 偶发 silent fail（退码 0 + 0 transferred） | — | 改 boto3 单对象 copy + 限并发 worker |
...
mc ls --versions <alias>/<bucket>/<prefix>/      # 列所有版本（§4 bug 下唯一可靠的枚举路径）
...
- 列举截断 bug：issue [rustfs/rustfs#3217](https://github.com/rustfs/rustfs/issues/3217)（复现 + 控制实验）、修复 PR [#3224](https://github.com/rustfs/rustfs/pull/3224)
