...
Copilot CLI 内部行为的逆向工程与排障笔记。涵盖：进程模型、bash 工具 env 处理、权限与目录信任、Walk-Up（向上查找）机制与各子系统（Custom Instructions / MCP / Skills / Hooks）、git 认证、TUI 渲染与滚动键（alt-screen / PageUp/PageDown 翻页）。

大部分章节附 `app.js` 源码摘录与字节偏移。源码定位基线为 `@github/copilot@1.0.64-1` 的 `app.js`（esbuild 混淆产物）。偏移和混淆符号名（`RR` / `Lg` / `O3` / `LB` …）都是 esbuild 产物，**每次发版都会变**，别照抄；要在新版本里重新定位，搜稳定的字面量（`COPILOT_RUN_APP` / `COPILOT_ALLOW_ALL` / `GITHUB_PERSONAL_ACCESS_TOKEN` / `safe.bareRepository` / `AGENTS.md` / `.mcp.json` …）。怎么把二进制发行版解包成可读 JS、再用 node 抠出这些片段，见下一节 [看源码：二进制发行版怎么解包](#看源码二进制发行版怎么解包)。

> **校准状态**：[进程模型](#进程模型)、[看源码](#看源码二进制发行版怎么解包)、[Env 黑名单](#env-黑名单为何-git-push-在-agent-里总是-401)、[TUI 渲染与滚动键](#tui-渲染与滚动键)、[终端颜色 / 主题（colorMode）](#终端颜色--主题colormode) 五节已按 1.0.64-1 实测核对，符号名/清单/可见信号均为现行版。**其余章节**（Walk-Up / Custom Instructions / Hooks / MCP / Skills / Git 认证 …）的机制锚点经确认仍在 1.0.64-1，但里面的混淆符号名（`bBt` / `vfi` / `xfi` / `Rfi` / `sN` …）和字节偏移**仍是早期版本的，未逐一重校**——读这些章节时以字面量锚点为准，符号名当线索别当事实。

## 目录

- [进程模型](#进程模型)
  - [三层结构：loader → app.js](#三层结构loader--appjs)
  - [看源码：二进制发行版怎么解包](#看源码二进制发行版怎么解包)
- [TUI 渲染与滚动键](#tui-渲染与滚动键)
...
入口因安装方式而异，但核心永远是 `index.js`（loader）→ spawn `app.js`（主逻辑）两层：

```
# npm 安装
~/.local/share/<node-install>/lib/node_modules/@github/copilot/
    ├── npm-loader.js   (shim, 转发到 index.js)
    ├── index.js        (loader)
    └── app.js          (主逻辑)

# 二进制发行版(SEA, 单可执行)
~/.local/bin/copilot                     ← 160MB stripped ELF, JS 内嵌
  首次运行自解包到 →
~/.cache/copilot/pkg/<platform>/<version>/
    ├── index.js / app.js / sea-loader.js
    ├── copilot-sdk/*.d.ts  (干净 TS 声明, 读类型面最省事)
    └── *.wasm / 各 native 模块
```

- `npm-loader.js`（仅 npm 安装有）只是把入口转发到 `index.js`。
- `index.js` 负责：自动更新、版本选择（`--prefer-version`）、crash report 收集。它用 `z({COPILOT_RUN_APP:"1", ...})` spawn `app.js` 子进程，`z()` 是个 `Proxy(process.env)` 包装，但**默认过滤集是空 Set**（`at=new Set`）——即 `index.js` 这层**不做** env 过滤。所以 `process.env.GITHUB_PERSONAL_ACCESS_TOKEN` 在 Copilot 主进程内部仍然可读（用来调内部 API、遥测鉴权等）。
- `app.js` 是主逻辑：tool registry / hook 解析 / MCP 客户端 / bash pty 工具的 env 过滤都在这一层。env 过滤是 app.js 这层**额外**做的（见 [Env 黑名单](#env-黑名单为何-git-push-在-agent-里总是-401)），与 index.js 的空过滤是两码事。
...
`copilot --version` 装的若是单可执行二进制（`file ~/.local/bin/copilot` 显示 `ELF ... stripped`，~160MB），JS 源码是内嵌的 [SEA](https://nodejs.org/api/single-executable-applications.html)（Single Executable Application，Node 把整个应用塞进一个可执行文件）。**不用自己拆 ELF**——CLI 首次运行时 `sea-loader.js` 会把内嵌资源解包成普通文件，落到：

```
~/.cache/copilot/pkg/<platform>/<version>/      # 如 linux-x64/1.0.64-1
```

目录里有同名 `.extraction-complete` 标记说明解包完成。直接 view 这些文件即可：

```bash
D=~/.cache/copilot/pkg/linux-x64/$(copilot --version | grep -oE '[0-9]+\.[0-9]+\.[0-9]+-[0-9]+')
ls "$D"                       # app.js index.js sea-loader.js copilot-sdk/ ...
wc -l "$D/app.js"             # 6403 行, 8.5MB —— esbuild 长行
```

`app.js` 是 esbuild bundle，**单行极长**（一行几 MB），grep 看不出上下文。实用做法是用 node 做字符串切片，以稳定字面量为锚点抠片段：
...
- **找配置/行为锚点**：用永不混淆的字符串常量（env 名、错误文案、配置文件名、CLI flag）。混淆符号名（函数/变量）每版都变，不能当锚点。
- **读类型面优先看 `copilot-sdk/*.d.ts`**：`types.d.ts`（60KB）、`client.d.ts`、`session.d.ts` 是没混淆的 TypeScript 声明，比从 `app.js` 反推结构清楚得多。
- npm 安装版没有这套 cache 解包，直接读 `node_modules/@github/copilot/app.js`（同样是 esbuild bundle，手法一致）。

---

## TUI 渲染与滚动键

Copilot CLI 是跑在 **alternate screen（备用屏，简称 alt-screen——像 vim/less 那样独占整屏、退出后恢复原终端内容）** 的全屏 TUI，自带一个 vim 风格的 pager（翻页器）。结论：**翻页用 PageUp / PageDown**，半页 `Ctrl+U` / `Ctrl+D`，到顶/底 `Home`(或 `g`/`gg`) / `End`(或 `G`)，`/` 搜索、`n`/`N` 跳下/上一个，也吃鼠标滚轮。

**关键坑**：alt-screen 没有终端回滚缓冲（scrollback），Copilot 还会打开鼠标追踪（`?1002h ?1006h`），所以**终端复用器（tmux / zellij 等）自己的"回滚滚动"在 Copilot 界面里什么都滚不动**——根本没东西可滚。要从外部驱动滚动，得把真正的 PageUp / PageDown **按键字节注入当前 pane**（用复用器的 send-keys / write 之类动作），而不是触发复用器的 scrollback：
...
**注意 `/help` 没列这两个键**（只列了 `ctrl+o/e 展开 timeline`），所以以源码为准。

源码锚点（`app.js`，1.0.64-1；搜稳定字面量定位）：

```js
// alt-screen 开关 + 鼠标追踪：进了 ?1049h 就没有 scrollback 了
ui = { ALT_SCREEN_ON:"\x1B[?1049h", ALT_SCREEN_OFF:"\x1B[?1049l", ... }
MOUSE_BUTTON:"\x1B[?1002h\x1B[?1006h"

// 输入解码表：把转义序列映射成键名（ESC[5~ → "pageup"）
"[5~":"pageup","[6~":"pagedown","[[5~":"pageup","[[6~":"pagedown"   // 另有 [5$ / [5^ 等变体

// pager handler：整页 / 半页 / 到顶到底
if(pt.code==="pageup"){qo(-xe);return}      // 上一整页（xe = 一页高度）
if(pt.code==="pagedown"){qo(xe);return}     // 下一整页
if(pt.ctrl&&pt.code==="u"){qo(-ha);return}  // 半页上（ha ≈ xe/2）
if(pt.ctrl&&pt.code==="d"){qo(ha);return}   // 半页下
// 同处还有 home / "g""gg" → 顶部，end / "G" → 底部
```

> 同样的 `pageup`/`pagedown` handler 在多个视图都出现（主内容视口、diff/review 查看器、列表选择器），所以"注入这两个键"是通用正确的滚动方式；具体某个面板用的是不是这套，需进对应界面实测。

---

## 终端颜色 / 主题（colorMode）

Copilot CLI 的 TUI 自带 **5 套真彩色（truecolor，直接发 RGB，不依赖终端 16 色）调色板**，由 `~/.copilot/settings.json` 的 `colorMode` 选：

```
default · github · dim · high-contrast · colorblind
```

源码锚点（`app.js`，1.0.64-1）：`FC=["default","github","dim","high-contrast","colorblind"]`，配置项 `colorMode:of(FC).describe("Color palette mode ...")`，默认 `ER="default"`。`github` 是 GitHub 官方 Dark & Light 配色，但被 feature-flag `COPILOT_GITHUB_THEME` 门控（staff/experimental 才在 `/themes` 里出现）。

### 主题是正交两层：colorMode × 明暗

排查"字发淡 / 主题不对"时必须分清这两层，它们独立：

1. **colorMode**（上面 5 个）决定"语义 → 色阶级别"的映射（源码里每个 mode 一个 `resolveTokens`，把 `textPrimary` 之类映射到某个色阶 `n(9)`）。
2. **明暗（light/dark）** 决定喂进去的色阶：同一个 `textPrimary`，浅色阶给深灰字、深色阶给浅灰字。

明暗不是手选的，是**探测**出来的：

- **主路**：发 **OSC 11**（`\x1B]11;?\x1B\\`，问终端默认背景色）→ 算 `luminance`（亮度）判明暗。
- **备路**：读 `COLORFGBG` 环境变量（形如 `fg;bg`，0-15 ANSI 色号，`bg=15` 白 = 浅）。

**关键坑（实测确认）**：`COLORFGBG` 只是 OSC 11 **探测失败时**的 fallback；OSC 11 **成功**（哪怕返回的是错值）就压过 `COLORFGBG`。所以当终端（如某些复用器）对 OSC 11 回了"黑背景"，手动设 `COLORFGBG="0;15"` 也救不回来——Copilot 拿到 OSC 11 的黑就判深色，用浅色字，落在浅背景上发淡。Copilot **没有**"直接钉死 light/dark"的开关（`appearance` 是内置 CSS 词表里的属性名，不是设置项），明暗只能靠这两条探测路。

> 典型场景：zellij web 浅色主题下 Copilot 字发淡 = 复用器 OSC 11 回黑被 Copilot 误判深色。根因与修复在 [zellij 参考的「终端默认背景（OSC 11）」一节](zellij.md)（zellij `≥0.44.3` 修了 web 模式 OSC 11 回黑）。换 `colorMode` 只换调色板映射、不改明暗判定，治标不治本。

### bash 工具拿不到用户的显示 tty

Copilot 的 bash 工具在一个 pty 里跑，但**接不到用户实际显示的那个 tty**（`/dev/tty` 不可用，报 `No such device or address`）。后果：**agent 自己发不了 OSC / DA 这类终端查询、也读不到终端应答**——任何"问终端背景色 / 能力探测 / 颜色诊断"的命令都得让用户**在交互 pane 里手动跑**，agent 只能给命令、读用户贴回来的结果。诊断终端颜色/主题问题时直接据此分工，别在 agent bash 里反复试 OSC。
...
```
bash 工具调用
  → sessionFactory.create({ env: RR({...}) })     // RR = 旧版 xj
  → pty.spawn(shell, args, { env })
```

#### 关键代码

> 符号名（`RR` / `Lg` / `LB` / `O3` / `Dft`）是 1.0.64-1 的 esbuild 产物，每版会变；锚点用代码里的字面量。

**env 构造入口（旧版叫 `xj`，1.0.64-1 是 `RR`）** — shell 工具构造 env：

```js
function RR(t = {}) {
  let e = new Set([...LB, ...Dft]);                 // 黑名单 = 内部上下文(LB) + electron 标志(Dft)
  let n = { ...t, COPILOT_CLI: "1" };
  // 白名单透传以下 GitHub/Copilot 内部变量：
  if (process.env.GITHUB_COPILOT_GITHUB_TOKEN)
    n.GITHUB_TOKEN = process.env.GITHUB_COPILOT_GITHUB_TOKEN;
  if (process.env.COPILOT_AGENT_CALLBACK_URL)   n.COPILOT_AGENT_CALLBACK_URL = ...;
  if (process.env.COPILOT_AGENT_JOB_ID)         n.COPILOT_AGENT_JOB_ID       = ...;
  if (process.env.GITHUB_VERIFICATION_TOKEN)    n.GITHUB_VERIFICATION_TOKEN  = ...;
  if (process.env.GITHUB_TOKEN_VARNAME)         n.GITHUB_TOKEN_VARNAME       = ...;
  if (process.env.GITHUB_COPILOT_API_TOKEN)     n.GITHUB_COPILOT_API_TOKEN   = ...;
  return Lg(n, e);
}
```
...
**Proxy 屏蔽函数（旧版 `_R`，1.0.64-1 是 `Lg`）** — 用 Proxy 模拟"剥离过的 env"：

```js
function Lg(t, e) {
  let n = Object.create(null);
  if (t) for (let [k, v] of Object.entries(t)) n[k] = v;
  let r = e ?? I5n;                                  // 默认黑名单 I5n
  return new Proxy(process.env, {
    get(_, s) {
      if (typeof s === "string") {
        if (Object.hasOwn(n, s)) return n[s];
        if (!r.has(s)) return process.env[s];        // ← 在黑名单 r 里直接返回 undefined
      }
    },
    set(_, s, a) { return n[s] = a, !0; },
    has(_, s) {                                       // env-name in 检查也过滤
      return typeof s !== "string" ? !1
        : Object.hasOwn(n, s) ? n[s] !== void 0
        : r.has(s) ? !1 : s in process.env;
    },
    ownKeys(_) { /* 黑名单 key 从 keys 里抹掉 */ },
    ...
  });
}
```

Node 的 `child_process.spawn` 在 `{env}` 是 Proxy 时会枚举 keys + 取值，黑名单 key 既不出现也读不到 → 子进程完全看不见。`index.js` 那层（[进程模型](#三层结构loader--appjs)）也用同构的 Proxy 包装，但默认黑名单是空 Set，所以只有 app.js 这层真正过滤。

**黑名单 `LB`（旧版 `vhe`）= 内部上下文变量 + `...O3`**，另有独立小名单 `Dft`：

```js
LB = [
  // Copilot 内部不应外泄的运行时上下文
  "COPILOT_AGENT_CALLBACK_URL", "COPILOT_AGENT_MODEL",
  "COPILOT_AGENT_REASONING_EFFORT",               // 1.0.64 新增
...
Dft = ["ELECTRON_RUN_AS_NODE"];                    // bash 黑名单 = new Set([...LB, ...Dft])

O3 = [                                             // 旧版 Nhe；24 项, 顺序即源码顺序
...
`O3` 同时被复用为日志 redaction（脱敏）名单和 MCP 配置 `${VAR}` 展开拦截名单（源码里 `for (let r of O3){...}` 收集 secret 值做日志打码，`new Set(O3)` + `filter(s => !n.has(s))` 拦 MCP 变量展开），是统一的「不该让 agent 看到的密钥」清单。
...
- `env | grep -i token` 在 agent bash 里看不到黑名单里那 24 个确切名字（`GITHUB_TOKEN` / `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` …），但你自己起名的 `*_TOKEN`（如 `MY_GH_TOKEN`、`WEBDAV_PASS`）照样透传 → 证实过滤是**按确切名单**，不是按 `*TOKEN*` 之类模式匹配
- `env | grep -i GIT_CONFIG` 会看到 Copilot 用 `GIT_CONFIG_COUNT` + `GIT_CONFIG_KEY_<n>` / `GIT_CONFIG_VALUE_<n>` 注入的几对 git config，**说明 spawn 时确实改写了 env**。实测 1.0.64-1 注入 4 对（`GIT_CONFIG_COUNT=4`）：

  ```
  GIT_CONFIG_KEY_0 = credential.https://github.com.helper   VALUE_0 = (空, 清掉已有 helper)
  GIT_CONFIG_KEY_1 = credential.https://github.com.helper   VALUE_1 = !f(){ ... }; f   (内联 helper)
  GIT_CONFIG_KEY_2 = core.excludesFile                       VALUE_2 = <某 excludes 文件>
  GIT_CONFIG_KEY_3 = safe.bareRepository                     VALUE_3 = explicit
  ```

  具体几对/哪些 key 由底层 `gitComputeSecurityEnvPatch` 动态算（实现不在 `app.js`，在更底层的打包模块，静态看不全）——**别照抄上面的清单，直接在 agent bash 里 `env | grep GIT_CONFIG` 看当前真实值**。其中 `credential.*.helper` 那两对若出现内联 helper，多半是你自己 direnv 注入的 [git push 对策](#bash-工具-env-黑名单下的-git-push-对策) 在生效，不是 Copilot 默认行为。
