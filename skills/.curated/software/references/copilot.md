...
> **校准状态**：[进程模型](#进程模型)、[看源码](#看源码二进制发行版怎么解包)、[Env 黑名单](#env-黑名单为何-git-push-在-agent-里总是-401) 三节已按 1.0.64-1 实测核对，符号名/清单/可见信号均为现行版。**其余章节**（Walk-Up / Custom Instructions / Hooks / MCP / Skills / Git 认证 …）的机制锚点经确认仍在 1.0.64-1，但里面的混淆符号名（`bBt` / `vfi` / `xfi` / `Rfi` / `sN` …）和字节偏移**仍是早期版本的，未逐一重校**——读这些章节时以字面量锚点为准，符号名当线索别当事实。
