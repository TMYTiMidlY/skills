...
> 本文档分两部分：**(A) REST API 与 admin token**（紧接下一节）—— 让 agent / 脚本编程操作网盘（ls / mv / cp / 上传 / 加减存储后端 / 看任务进度），比操控网页可靠；**(B) 网盘部署运维坑** —— iCloud / SMB 集成与网络测速，从「坑分类速查表」往下。
...
定位思路：从进程拿真实 exe 路径与属主（PowerShell `Get-CimInstance Win32_Process` 的 `ExecutablePath` + `GetOwner` / `SessionId`），再去那个用户的 `AppData\Roaming\OpenList Desktop\` 找 `data\`。本机的 exe 路径 / data 目录 / 访问入口等具体值存在工作区 `.envrc` 的 `OPENLIST_*`（私货，不进 skill 仓库）。

### 安全：token 不进 agent 的 context

固定 admin token 权限极大，取它时让明文经过 agent 的对话 / 工具输出 / 命令 argv 都会留痕。一个可行模式：经 SSH 加密隧道在远端跑 CLI，输出**直接管道进密钥库**（如 portal-mcp-server 的 `secrets_store.send_secret`），agent 侧只看到长度 / 状态。要点：

- 远端调 Windows 程序时，用 PowerShell `-EncodedCommand`（base64 的 UTF-16LE）规避多层引号转义；
- token 只在脚本内存流转，不 `echo`、不写盘、不进命令行参数；
- 之后 API 请求用密钥库的 secret 注入机制（值在返回里被 redact）—— 见 remote / portal skill。

### REST API 速用

- **base url**：`<api-base>`（本机值见 `.envrc` 的 `OPENLIST_API_BASE`）。
