...
因为提交上限包含页面文件，所以"虚拟机上限 + Windows 需求 > 物理内存"可以合法通过，Windows 全程认为自己工作正常。定 `memory=` 时按物理内存减去宿主实测占用来算，这个和不交给任何自动机制把关。
...
- **页面文件（pagefile）**：Windows 自己的交换文件（`C:\pagefile.sys`），服务 Windows。它与 WSL 的 swap 是**两套独立机制**，前者归宿主，后者归 guest，互不替代，也不要为了给 WSL 腾空间去关掉它。
...
性能上，guest 的一次磁盘 I/O 要穿过 guest 块设备层 → VHDX → 宿主文件系统 → 物理盘，比裸盘多几层。顺序大块读写影响有限，**随机小 I/O 惩罚明显**——而换页恰好是随机小 I/O。

## <a id="page-fault"></a>缺页与访问延迟

内存以**页**为单位管理（通常 4 KiB）。程序访问的是虚拟地址，CPU 需要查页表翻译成物理地址；若发现该页当前不在物理内存里（被换到 swap 了），就触发一次**缺页（page fault）**中断交给内核：内核把这一页从磁盘读回内存、更新页表，然后 CPU 重新执行刚才那条指令。
