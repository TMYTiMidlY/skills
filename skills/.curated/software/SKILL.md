...
Mihomo / Clash 客户端配置与泄漏控制、WSL ↔ Windows ↔ 远端的网络管道（RDP、WSL Mirror/NAT、VS Code serve-web、portproxy/wslrelay 入站、出站走宿主 Mihomo、EasyTier 组网）、以及独立 Hysteria2 服务端搭建，已抽出到独立的 `network` skill（见其 `references/mihomo.md` 与 `references/network.md`）。
...
> WSL 内 Linux 服务的网络坑（wslrelay、portproxy、Mihomo TUN、Docker IPv6 dual-stack）在 `network` skill 的 `references/network.md`，不在 windows.md 重复。
