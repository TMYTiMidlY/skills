...
工作区同时躺着"你想动的改动"和"不该由你带走的改动"（并发会话未提交的脏文件、别人已 `git add` 的 rename、untracked 文件）时，如何在**命令行非交互**地只对一处做 commit/stage/discard/stash，以及提交被别人叠了新提交后怎么改它。先讲**地基**：Git **三棵树**（工作区/暂存区 index/HEAD，出处 Pro Git《Reset Demystified》）、`git commit`（无路径）= 给整个 index 拍快照（暂存区里什么都必带走）、任何"部分操作"= 拿两棵树做 diff（谁是底座 floor 决定动得了什么）。正文按**粒度**分两大场景——**整文件级**：`git commit -- <path>`（不带 -p 的 pathspec 提交，忽略整个 index、只记 HEAD+该文件当前内容，连别人已暂存的 rename 都不搭车；全新文件先 `git add -N -- <path>` 只写 intent-to-add 标记；丢弃/撤出/搬走整文件用 `git restore -- / --staged -- / -SW --source=HEAD --`、`git stash push --`；`--` 消歧符加不加结果相同但推荐带上）、及"加了 -p 反而破坏隔离性"的反直觉坑（`commit -p f.txt` 仍带走已暂存内容）；**子文件 hunk/行级**：非交互**补丁手术**（`git diff | filterdiff --hunks=N | git apply --cached` 一行式，或手删补丁 `@@` 段、`git apply -R` 反向丢弃/撤暂存），及**交互式 `-p`**（**不是 TUI**，是行式提示、能被 `printf 'y\n' | git add -p` 管道驱动；各 `-p` 命令**基准对照表**：`add/commit/restore -p` 基准=index 只动未暂存、`stash -p` 与 `restore -SW --source=HEAD -p` 基准=HEAD 才够得到已暂存，`stash -p` 不重置 index；`y/n/a/d/s/e` 按键、split 要有未改动行才拆、edit 到行级）；**后面已有提交时如何 amend**——`git rebase -i` 的 `reword`（保 tree 只换消息、被改条及其上子提交全部换新 SHA）语义，以及工作区被并发脏文件占住、rebase 拒绝启动时的 plumbing 等效法（`commit-tree` 复用原 tree 重建 + `update-ref` 带旧值 CAS 原子移分支、不碰 index/worktree）见 [references/git-surgery.md](references/git-surgery.md)。

## jj（Jujutsu）：working copy 即 commit、操作日志、一等冲突、与 Git 互操作

jj 用 Git 仓库当后端、协作者可无感，但模型和 Git 不同。覆盖：**working copy 即自动提交的 `@`、没有暂存区**（每条命令先快照 `@` 再干活，`.git/index` 被忽略、不跑 git hooks）；**两条独立历史**——提交图 `jj log`（change/commit DAG）与**操作日志 `jj op log`**（每条改仓库的命令一个节点、含整仓快照视图，`jj undo` / `jj op restore <op>` 撤销回退、`jj --at-op=<op>` 只读回看；纯本地不随 push 走），及"想 diff/回退到没建成 commit 的中间态只能走 op、`jj diff --from <op>` 不认 op"的推论；**一等冲突**（rebase 撞冲突**不阻塞**、冲突当数据存进 commit、`× (conflict)` 挂着照样干活、按需再 `jj edit` 解，省掉 `git rebase --continue` 那套，auto-rebase 后代）；**冲突与 change-id 在 Git 层的落地**（冲突 commit 的 git 树是单边 materialize + `.jjconflict-base/side-*/` 目录 + `JJ-CONFLICT-README`，权威信息在非标准 `jj:trees` header、`refs/jj/keep/*` 防 GC、冲突分支标成 `branch@git`；change-id 写进非标准 commit header、`git show`/`log` 不显示但 `git cat-file` 能看到、过普通 `git rebase` 会丢、`git.write-change-id-header` 可关）；**哪些数据留在本地 vs 随 `jj git push` 传播**（op log/快照/abandon 的实验/重写全本地，只有 push/fetch/clone 碰远端；`jj git push` 拒推 conflict / 无描述 / private / 缺 author-committer 的 commit；`jj git clone` 只搬 commit+bookmark+tags、op log 全新，拷整个仓库目录（含 `.jj` 与同级 `.git`）才带全过程、经 rsync/Dropbox 可无锁合并；远端只能是 git 远端、无 jj 原生同步协议）；**文件不丢失与恢复**（工作副本改动经下一条命令快照进 `@`、每步进 op log 连破坏性操作都可撤、`refs/jj/` 钉住被弃 commit、op log 可 `jj op abandon`+`jj util gc` 修剪）**及其边界**（`.jj` 本地明文非加密、不随 push 走、不是异地备份）。**实测 jj 0.43.0**。见 [references/jj.md](references/jj.md)。
