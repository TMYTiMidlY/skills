...
## jj（Jujutsu）：无暂存区模型与 Git 互操作

jj 用 Git 仓库当后端、协作者可无感，但模型和 Git 不同——遇到 jj 的工作副本 / 操作日志 / 冲突 / Git 互操作 / 文件持久性问题看这篇。按 jj.md 的章节：**工作副本 `@` 与暂存区**（无 index、命令懒快照 `@`）、**操作日志与提交图**（`jj op log` vs `jj log` 两条独立历史、`jj undo` / `op restore` / `--at-op`、op log 纯本地不随 push）、**一等冲突**（rebase 不阻塞、冲突进 commit、按需再解、auto-rebase 后代）、**冲突与 change-id 在 Git 层的表示**（`.jjconflict-*` / `jj:trees` / `refs/jj`、change-id header 泄进 git）、**留在本地 vs 随 push 传播的数据**（clone 只带 commit+bookmark+tags、op log 不走；备份要拷整个仓库目录含同级 `.git`）、**改动的持久性与恢复**（懒快照时机、`jj op abandon`+`jj util gc` 修剪、明文非备份的边界）、**与纯 Git 工具混用的限制**（hooks / staging / detached HEAD）。实测 jj 0.43.0。见 [references/jj.md](references/jj.md)。
