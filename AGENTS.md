...
- 在 Git 仓库内，优先复用项目已有的合适临时目录及忽略规则。需要新增忽略规则时，**先征得用户同意**：个人使用可写入 `.git/info/exclude`，团队共享约定再修改 `.gitignore`。忽略路径按仓库根目录计算；工作区就是仓库根时，对应规则为 `/.agent-tasks/`。
- Git 忽略规则不会取消已有文件的跟踪，也不会扩大写权限；若 Git 元数据位于工作区之外，仍须走正常授权。任务目录（包括 `outputs/`）默认不纳入项目提交；用户要求将交付物纳入项目时，再明确保存位置与提交范围。
- 不因任务结束而自动清理任务目录，也不擅自移动或整理已有文件；需要清理时遵循用户指示及本文件的删除规则。
