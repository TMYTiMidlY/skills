---
name: graft-skill
description: 管理从外部 GitHub 仓库嫁接（引入）的 skill。当用户要添加、移除、修改外部 skill，或提到检查更新、同步上游、upstream 时触发。
---
...
**Agent 边界：遇到不确定必须问用户，不要瞎猜**——re-graft / 解冲突 / 重写 patch 时，下面这些情况 agent 必须停下来征求用户决定，不要自作主张：

- 上游重命名/移动/删除了本地 patch 触及的文件、函数、配置项，**该把本地适配挂到新位置还是放弃这条适配？**
- 上游引入了与本地 patch 解决同一问题的新机制（例如本地 patch 是把硬编码路径改成 env var，上游新增了另一套 config 字段做类似事），**本地 patch 该淘汰、合并还是并存？**
- 上游新增的子系统（新目录、新文件类型）**是否也要纳入本地 patch 的适配范围**（例如本地 patch 把 `templates/{brands,layouts}` 外置，上游新增了 `templates/decks`，要不要也外置？）
- patch 触及多个相关文件，逐个手解后语义可能漂移——**全做完后是否需要用户回看整体逻辑一致性？**
- patch 描述里写的设计意图与新上游代码现状产生分歧——**坚持原意图还是顺应上游？**

这些是设计/产品决策，不是机械合并。Agent 看到这类信号要主动列出选项 + 取舍给用户选，避免静默吞掉判断。
